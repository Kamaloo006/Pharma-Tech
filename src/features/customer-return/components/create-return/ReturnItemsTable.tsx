    ...
    <div className="bg-card border border-border/60 rounded-xl p-5 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-2 border-b border-border/60">
        <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
          ...
          {t("customerReturn.create.returnItems", "Items to Return")}
        </h2>
      </div>
          ...
          <TableHeader className="bg-muted/30">
            <TableRow className="border-b border-border/60">
              <TableHead className={isArabic ? "text-right" : "text-left"}>
                {t("customerReturn.create.product", "Product")}
              </TableHead>
              <TableHead className="text-center">
                {t("customerReturn.create.purchased", "Purchased Qty")}
              </TableHead>
              <TableHead className="text-center w-32">
                {t("customerReturn.create.returnQty", "Return Qty")}
              </TableHead>
              <TableHead className="text-center">
                {t("customerReturn.create.unitPrice", "Unit Price")}
              </TableHead>
              <TableHead className="text-center">
                {t("customerReturn.create.total", "Total")}
              </TableHead>
          ...
          <TableBody>
            {items.length === 0 ? (
              <TableRow>
                ...
                >
                  {t(
                    "customerReturn.create.noItems",
                    "No items available or invoice not selected",
                  )}
                </TableCell>
              </TableRow>
            ) : (
