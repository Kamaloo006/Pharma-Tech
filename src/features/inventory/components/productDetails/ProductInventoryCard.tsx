// components/productDetails/ProductInventoryCard.tsx
import {
  ShoppingCart,
  PlusCircle,
  AlertCircle,
...
import { Button } from "@/components/ui/button"; // تأكد من وجود زر Shadcn
import { useNavigate } from "react-router-dom"; // لتوجيه الموظف لصفحة المشتريات
...
interface ProductInventoryCardProps {
  ...
  isOut: boolean;
  isLowStock: boolean;
  onManualAdjustClick?: () => void; // تمرير ميزة فتح مودال التسوية اليدوية الذي بنيناه
}
  ...
  return (
    ...
    >
      <h3 className="text-xs font-semibold tracking-wider text-muted-foreground uppercase border-b border-border pb-2 text-start">
        {isArabic ? "حالة المخزون المستودعي" : "INVENTORY STATUS"}
      </h3>
      ...
      <div className="flex items-center justify-between text-xs py-2 border-t border-b border-border/40">
        <span className="text-muted-foreground">
          {isArabic ? "الحالة الحالية:" : "Status:"}
        </span>
        <span
      ...
      {isQuantityZero ? (
        // الحالة الأولى: عندما يكون المخزون 0 تماماً
        <div className="space-y-3 pt-1">
          <div className="flex gap-2 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10 text-[11px] leading-relaxed text-start text-muted-foreground">
            ...
            <p>
              {isArabic
                ? "هذا المنتج غير متوفر حالياً. يمكنك إنشاء فاتورة شراء لإعادة تعبئة المخزون، أو استخدام التعديل اليدوي فقط في الحالات الاستثنائية."
                : "This product is currently out of stock. You can create a purchase invoice to replenish it, or use manual adjustment for exceptional cases only."}
            </p>
          </div>

          <div className="flex flex-col gap-2">
            <Button
              onClick={() => navigate("/purchases/new")} // يوجه لصفحة إنشاء الفاتورة الجديدة
            ...
            >
              ...
              {isArabic ? "إنشاء فاتورة شراء" : "Create Purchase Invoice"}
            </Button>

            <Button
              variant="outline"
              onClick={onManualAdjustClick} // يفتح مودال التسوية اليدوية الاحترافي
            ...
            >
              ...
              {isArabic ? "تعديل يدوي استثنائي" : "Manual Adjustment"}
            </Button>
          </div>
        </div>
      ) : (
        // الحالة الثانية: عندما يكون المخزون أكبر من 0
        <div className="pt-1">
          <Button
          ...
          >
            ...
            {isArabic ? "استلام توريد جديد" : "Receive New Purchase"}
          </Button>
        </div>
      )}
    </div>
